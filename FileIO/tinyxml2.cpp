...
{
	_bufferOutSize = 10;
	_bufferOut = (char*) malloc(_bufferOutSize);
}
...
char* XMLElement::CStr() {
	XMLPrinter printer;
	this->Print(( XMLPrinter*)&printer);
	if(_bufferOutSize < printer.CStrSize()) {
		_bufferOut = (char*) realloc(_bufferOut, printer.CStrSize());
		_bufferOutSize = printer.CStrSize();
	}
	memcpy(_bufferOut, printer.CStr(), printer.CStrSize());
	return _bufferOut;
}

int XMLElement::CStrSize() {
