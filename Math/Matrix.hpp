...
namespace Math {
    ...
    // template on http://www.drdobbs.com/a-c-matrix-template-class/184403323
    ...
    public:
        ...
        matrix(unsigned rows, unsigned cols, const T* elements);
        ...
        // determinants
        matrix<T> matrix_minor(unsigned i, unsigned j) const;
        T det() const;
	...
	: rows(rows), cols(cols), elements(rows*cols, T()) {
        matrix(rows, cols, 0);
    };
